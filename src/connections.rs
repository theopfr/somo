...
#[cfg(target_os = "macos")]
use netstat2::{get_sockets_info, AddressFamilyFlags, ProtocolFlags, ProtocolSocketInfo as NetstatSocketInfo};
#[cfg(target_os = "macos")]
...
fn get_connections_macos(filter_options: &FilterOptions) -> Vec<Connection> {
    ...
    sockets_info
        .iter()
        .filter_map(|si| {
            let (proto, local_port, remote_address, remote_port, state) = match &si.protocol_socket_info {
...
pub fn get_all_connections(filter_options: &FilterOptions) -> Vec<Connection> {
    ...
    #[cfg(target_os = "macos")]
    {
        get_connections_macos(filter_options)
    }
}
